...
public class ReportOutcome {
	...
	void onEventDidOccur() {
		...
		alertManager.alert(Duration.TRANSIENT, Severity.SUCCESS, "Event set as occured, you may leave the page.");
	}
	
	void onEventDidNotOccur() {
		exercise.setOutcome(false);
		exerciseDatabaseService.updateExercise(exercise);
		alertManager.alert(Duration.TRANSIENT, Severity.SUCCESS, "Event set as not-occured, you may leave the page.");
	}
}
